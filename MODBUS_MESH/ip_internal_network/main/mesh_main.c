...
static esp_err_t slave_init(mb_communication_info_t* comm_info, esp_netif_t *netif)
{
...
esp_err_t esp_mesh_comm_mqtt_task_start(void)
{
    ...
    if (!is_comm_mqtt_task_started) {
        xTaskCreate(slave_operation_func, "modbus task", 3072, NULL, 5, NULL);
...
mb_communication_info_t comm_info = { 0 };
    ...
    ESP_ERROR_CHECK(slave_init(&comm_info, &netif));
