 ...
 *******************************************************/
// interaction with mesh for charging data
void change_charging_data(char* str);
...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            if (esp_mqtt_client_subscribe(s_client, "/charging", 0) < 0) {
        ...
        case MQTT_EVENT_DATA:
            ...
            char str[5]; // 5 for now but later dynamically allocated by number of connected chargers
            sprintf(str, "%.*s", event->data_len, event->data);
            ESP_LOGI(TAG, "DATA=%s", str);
            change_charging_data(str);
            break;
