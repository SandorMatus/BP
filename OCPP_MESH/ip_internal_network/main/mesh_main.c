...
void esp_mesh_mesh_task(void *arg)
{
    ...
    ao_bootNotification("ESP-IDF charger", "Advantech - NODE", NULL, NULL, NULL, NULL); //send first OCPP message
    wifi_ap_record_t ap;
...
esp_err_t esp_mesh_comm_mqtt_task_start(void)
{
    ...
    if (!is_comm_mqtt_task_started) {
        xTaskCreate(esp_mesh_mesh_task, "mesh task", 10000, NULL, 5, NULL);
        //xTaskCreate(esp_mesh_ocpp_task, "ocpp task", 3072, NULL, 7, NULL);
...
void app_main(void)
{
    ...
    ESP_LOGI(MESH_TAG, "mesh starts successfully, heap:%ld, %s\n",  esp_get_free_heap_size(),
